...
#         response = self.client.post(self.friend_url, {'user_id': self.user2.user_id}, HTTP_AUTHORIZATION=self.user1.access_token)
#         print("Response: ", response.content)
#         self.assertEqual(response.status_code, 200)
#         self.assertTrue(Friends.objects.filter(user1=self.user1, user2=self.user2).exists())